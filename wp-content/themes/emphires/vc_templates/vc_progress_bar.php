...
foreach ( $graph_lines_data as $line ) {
	...
	$output .= '<div class="vc_general vc_single_bar' . ( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ?
		...
		. '">';
	$label = '<span class="cspt-pbar-title">' . $line['label'] . '</span>';  // CSPT Modified
	$output .= '<small class="vc_label"' . $line['txtcolor'] . '>' . $label /* CSPT Modified */ . $unit . '</small>';
	if ( $max_value > 100.00 ) {
		$percentage_value = (float) $line['value'] > 0 && $max_value > 100.00 ? round( (float) $line['value'] / $max_value * 100, 4 ) : 0;
	} else {
		$percentage_value = $line['value'];
	}
	$output .= '<span class="vc_bar-wrapper"><span class="vc_bar ' . esc_attr( implode( ' ', $bar_options ) ) . '" data-percentage-value="' . esc_attr( $percentage_value ) . '" data-value="' . esc_attr( $line['value'] ) . '"' . $line['bgcolor'] . '></span></span>';  // CSPT Modified
	$output .= '</div>';
}
$output .= '</div>';
echo cspt_esc_kses($output);
