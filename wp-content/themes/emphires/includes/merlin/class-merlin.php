...
class Merlin {
	...
	public function admin_page() {
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'emphires' ),
		);
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return esc_html__( 'Please define default parameters in the form of an array.', 'emphires' );
		}
		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return esc_html__( 'Please define an SVG icon filename.', 'emphires' );
		}
		// Set defaults.
		$defaults = array(
			'icon'        => '',
	...
	public function steps() {
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'emphires' ),
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'emphires' ),
				'view' => array( $this, 'content' ),
			);
		}
		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'emphires' ),
	...
	<?php
		$this->logger->debug( esc_html__( 'The welcome step has been displayed', 'emphires' ) );
	}
	/**
	...
	protected function license() {
		...
		<?php
		$this->logger->debug( esc_html__( 'The license activation step has been displayed', 'emphires' ) );
	}
	/**
	...
	<?php
		$this->logger->debug( esc_html__( 'The child theme installation step has been displayed', 'emphires' ) );
	}
	/**
	 * Theme plugins
	 */
					...
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							...
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								...
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_html_e( 'Required', 'emphires' ); ?>">
										<?php esc_html_e( 'req', 'emphires' ); ?>
									</span>
								</span>
							</label>
						</li>
	...
	<?php
		$this->logger->debug( esc_html__( 'The plugin installation step has been displayed', 'emphires' ) );
	}
	/**
	 * Page setup
	 */
	protected function content() {
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				<div class="merlin__select-control-wrapper">
					...
					<div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'emphires' ); ?>">
		...
		<form action="" method="post" class="<?php echo esc_attr( $multi_import ); ?>">
			<ul class="merlin__drawer merlin__drawer--import-content js-merlin-drawer-import-content">
				<?php echo cspt_esc_kses( $this->get_import_steps_html( $import_info ) ); ?>
			</ul>
			<footer class="merlin__content__footer">
				<a id="close" href="<?php echo esc_url( $this->step_next_link() ); ?>" class="merlin__button merlin__button--skip merlin__button--closer merlin__button--proceed"><?php echo esc_html( $skip ); ?></a>
				<a id="skip" href="<?php echo esc_url( $this->step_next_link() ); ?>" class="merlin__button merlin__button--skip merlin__button--proceed"><?php echo esc_html( $skip ); ?></a>
	...
	<?php
		$this->logger->debug( esc_html__( 'The content import step has been displayed', 'emphires' ) );
	}
	/**
	 * Final step
	 */
	protected function ready() {
		...
		<footer class="merlin__content__footer merlin__content__footer--fullwidth <?php echo esc_attr( $links_class ); ?>">
			<a href="<?php echo esc_url( $this->ready_big_button_url ); ?>" class="merlin__button merlin__button--blue merlin__button--fullwidth merlin__button--popin" target="_blank"><?php echo esc_html( $big_btn ); ?></a>
			<?php if ( ! empty( $links ) ) : ?>
				<a id="merlin__drawer-trigger" class="merlin__button merlin__button--knockout"><span><?php echo esc_html( $action ); ?></span><span class="chevron"></span></a>
	...
	<?php
		$this->logger->debug( esc_html__( 'The final step has been displayed', 'emphires' ) );
	}
	/**
	...
	public function generate_child() {
		...
		if ( ! file_exists( $path ) ) {
			...
			$this->logger->debug( esc_html__( 'The existing child theme was activated', 'emphires' ) );
			wp_send_json(
				array(
					'done'    => 1,
					'message' => sprintf(
		...
		$this->logger->debug( esc_html__( 'The newly generated child theme was activated', 'emphires' ) );
		wp_send_json(
			array(
				'done'    => 1,
				'message' => sprintf(
	...
	public function _ajax_activate_license() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'emphires' ),
				)
			);
		}
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'emphires' ),
				)
			);
		}
		...
		$this->logger->debug( esc_html__( 'The license activation was performed with the following results', 'emphires' ), $result );
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'emphires' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					case 'expired':
						$message = sprintf(
							...
							esc_html__( 'Your license key expired on %s.', 'emphires' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;
					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'emphires' );
						break;
					case 'missing':
						$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'emphires' );
						break;
					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'emphires' );
						break;
					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'emphires' ), $this->edd_item_name );
						break;
					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'emphires' );
						break;
					default:
						$message = esc_html__( 'An error occurred, please try again.', 'emphires' );
						break;
	...
	public function generate_child_functions_php( $slug ) {
		...
		$this->logger->debug( esc_html__( 'The child theme functions.php content was generated', 'emphires' ) );
		// Filterable return.
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		// Let's remove the tabs so that it displays nicely.
		$output = trim( preg_replace( '/\t+/', '', $output ) );
		$this->logger->debug( esc_html__( 'The child theme style.css content was generated', 'emphires' ) );
	...
	public function generate_child_screenshot( $path ) {
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			$this->logger->debug( esc_html__( 'The child theme screenshot was copied to the child theme, with the following result', 'emphires' ), array( 'copied' => $copied ) );
		} else {
			$this->logger->debug( esc_html__( 'The child theme screenshot was not generated, because of these results', 'emphires' ), array( 'screenshot' => $screenshot ) );
		}
	}
	/**
	 * Do plugins' AJAX
	...
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'emphires' ),
				);
				break;
			}
		}
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'emphires' ),
				);
				break;
			}
		}
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'emphires' ),
				);
				break;
			}
		}
		if ( $json ) {
			$this->logger->debug(
				esc_html__( 'A plugin with the following data will be processed', 'emphires' ),
				array(
			...
			$json['message'] = esc_html__( 'Installing', 'emphires' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
				esc_html__( 'A plugin with the following data was processed', 'emphires' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'emphires' ),
				)
			);
		}
		exit;
	...
	function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			$this->logger->error( esc_html__( 'The content importer AJAX call failed to start, because of incorrect data', 'emphires' ) );
			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'emphires' ),
				)
			);
		}
		$json         = false;
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				$this->logger->info(
					esc_html__( 'The content import AJAX call will be executed with this import data', 'emphires' ),
					array(
		...
		if ( $json ) {
			...
			$this->logger->error(
				esc_html__( 'The content import AJAX call failed with this passed data', 'emphires' ),
				array(
			...
			wp_send_json(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'emphires' ),
					'logs'    => '',
					'errors'  => '',
				)
			);
	...
	public function _ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			$this->logger->error( esc_html__( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'emphires' ) );
			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'emphires' ),
				)
			);
		}
		$selected_import = intval( $_POST['selected_index'] );
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title'            => esc_html__( 'Content', 'emphires' ),
				'description'      => esc_html__( 'Demo content data.', 'emphires' ),
				'pending'          => esc_html__( 'Pending', 'emphires' ),
				'installing'       => esc_html__( 'Installing', 'emphires' ),
				'success'          => esc_html__( 'Success', 'emphires' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'emphires' ),
				'description'      => esc_html__( 'Sample widgets data.', 'emphires' ),
				'pending'          => esc_html__( 'Pending', 'emphires' ),
				'installing'       => esc_html__( 'Installing', 'emphires' ),
				'success'          => esc_html__( 'Success', 'emphires' ),
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'emphires' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'emphires' ),
				'pending'          => esc_html__( 'Pending', 'emphires' ),
				'installing'       => esc_html__( 'Installing', 'emphires' ),
				'success'          => esc_html__( 'Success', 'emphires' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'emphires' ),
				'description'      => esc_html__( 'Sample theme options data.', 'emphires' ),
				'pending'          => esc_html__( 'Pending', 'emphires' ),
				'installing'       => esc_html__( 'Installing', 'emphires' ),
				'success'          => esc_html__( 'Success', 'emphires' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'emphires' ),
				'description'      => esc_html__( 'Redux framework options.', 'emphires' ),
				'pending'          => esc_html__( 'Pending', 'emphires' ),
				'installing'       => esc_html__( 'Installing', 'emphires' ),
				'success'          => esc_html__( 'Success', 'emphires' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'emphires' ),
				'description'      => esc_html__( 'After import setup.', 'emphires' ),
				'pending'          => esc_html__( 'Pending', 'emphires' ),
				'installing'       => esc_html__( 'Installing', 'emphires' ),
				'success'          => esc_html__( 'Success', 'emphires' ),
	...
	public function import_revolution_sliders( $file ) {
		...
		if ( class_exists( 'RevSlider' ) ){
			...
			$slider_array	= array(
				WP_PLUGIN_DIR . '/emphires-addons/demo-content/sliders/homeslider1.zip',
				WP_PLUGIN_DIR . '/emphires-addons/demo-content/sliders/homeslider2.zip',
				WP_PLUGIN_DIR . '/emphires-addons/demo-content/sliders/homeslider3.zip',
				WP_PLUGIN_DIR . '/emphires-addons/demo-content/sliders/productslider.zip',
			);
		...
		$this->logger->info( esc_html__( 'The revolution slider import was executed', 'emphires' ) );
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Installing', 'emphires' );
		$data['proceed']  = 'true';
	...
	public function after_content_import_setup() {
		...
		if ( $homepage ) {
			...
			$this->logger->debug( esc_html__( 'The home page was set', 'emphires' ), array( 'homepage_id' => $homepage ) );
		}
		...
		if ( $blogpage ) {
			...
			update_option( 'show_on_front', 'page' );
			$this->logger->debug( esc_html__( 'The blog page was set', 'emphires' ), array( 'blog_page_id' => $blogpage ) );
		}
	}
	/**
	...
	public function before_content_import_setup() {
		...
		if ( ! empty( $hello_world ) ) {
			...
			$this->logger->debug( esc_html__( 'The Hello world post status was set to draft', 'emphires' ) );
		}
	}
	/**
	...
	public function validate_import_file_info( $import_files ) {
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				$this->logger->warning( esc_html__( 'This predefined demo import does not have the name parameter: import_file_name', 'emphires' ), $import_file );
			}
		}
