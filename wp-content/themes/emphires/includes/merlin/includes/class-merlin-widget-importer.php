	...
	private static function process_import_file( $file ) {
		...
		if ( ! file_exists( $file ) ) {
			return new \WP_Error(
				...
				esc_html__( 'Error: Widget import file could not be found.', 'merlin-wp' )
			);
		}
		...
		if ( empty( $data ) ) {
			return new \WP_Error(
				...
				esc_html__( 'Error: Widget import file does not have any content in it.', 'merlin-wp' )
			);
		}
	...
	private static function import_data( $data ) {
		...
		if ( empty( $data ) || ! is_object( $data ) ) {
			return new \WP_Error(
				...
				esc_html__( 'Error: Widget import data could not be read. Please try a different file.', 'merlin-wp' )
			);
		}
		// Hook before import.
